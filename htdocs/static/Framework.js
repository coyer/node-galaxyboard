...
//  Boardflags (f�r das Board selbst)
//  PAB-Konfiguration
...
var dft_mod_closed      =   1<<1    //  [done]  Topic geschlossen durch admin (kann dann auch nur durch admin ge�ffnet werden)
...
var dfu_openpm          =   1<<4    //  REMX    Popup �ffnen wenn neue Nachrichten
...
var dfu_invalid         =   1<<28   //  REMX    EMail-Adresse des Users noch nicht gepr�ft!
...
var dfbf_bbcode_rule    =   1<<3    //  -obs-   Ob die Regeln bbcode enthalten d�rfen
var dfbf_smilies_rule   =   1<<4    //  -obs-
var dfbf_links_rule     =   1<<5    //  -obs-

var dfbf_showsubboards  =   1<<6    //          Ob untergeordnete Boards angezeigt werden d�rfen
...
var dfbf_nonews         =   1<<10   //          Beitr�ge nicht in News erw�hnen
var dfbf_dynmenu        =   1<<11   //  -obs-   dynamische men�s!
...
var dfbp_incpostcounter =   1<<(50-32)   //         Falls abgeschaltet wird Postcounter nicht erh�ht
...
//  Beitr�ge
var dfmod_approvepost   =   1<<0    //  -obs-   Kann Beitr�ge freigeben
var dfmod_setauthor     =   1<<1    //  -obs-   Kann Autor eines Beitrags �ndern
var dfmod_deletepost    =   1<<2    //  [done]  Kann Beitr�ge l�schen
var dfmod_editpost      =   1<<3    //  [done]  Kann Beitr�ge �ndern
var dfmod_closereports  =   1<<4    //  [done]  Kann Meldungen schlie�en und l�schen
...
var dfmod_jointhreads   =   1<<17   //  -obs-   Kann Themen zusammenf�hren
var dfmod_movethread    =   1<<18   //  [done]  Kann Themen verschieben
var dfmod_splitthread   =   1<<19   //  -obs-   Kann Themen teilen
var dfmod_deletethread  =   1<<20   //  [done]  Kann Themen l�schen
...
var dfpost_needapproval =   1<<0    //          Beitrag ben�tigt Freigabe (ggf. auch Topic mit freigeben falls themenstarter?)
var dfpost_hide         =   1<<1    //  [done]  Beitrag wurde versteckt
var dfpost_reported     =   1<<2    //  [done]  Beitrag wurde gemeldet (darf nicht mehr ge�ndert werden)


//  ---------------------------------------------------
//  Prototypen Hilfsfunktionen:
function $(id) {return document.getElementById(id);}
function floatval(t) {var z = parseFloat(t,10); if (isNaN(z)) { z = 0.0;} return z;}
function intval(t)   {var z = parseInt(t,10);   if (isNaN(z)) { z = 0;}   return z;}

String.prototype.trim   = function() { return this.replace(/^\s+|\s+$/g,""); }
String.prototype.ltrim  = function() { return this.replace(/^\s+/,""); }
String.prototype.rtrim  = function() { return this.replace(/\s+$/,""); }
String.prototype.pad    = function(l){ if(typeof(l)=="undefined")l="0"; return( l.substr(0, (l.length-this.length) )+this ); }
String.prototype.escape = function() { return this.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"); }
String.prototype.jsstring   = function() { return this.replace(/\'/g, "\\'"); }
String.prototype.startswith = function(search) { return (this.substr(0, search.length)==search)?true:false; }
String.prototype.endswith   = function(search) { return (this.substr(this.length-search.length)==search)?true:false; }
...
function debug(txt) {try { console.log(txt); } catch(e) {}}
...
function  CGalaxyboard()
{
    ...
    var mThreads        =   {};
    var mTopics         =   {};
    var mPosts          =   {"topicID": 0, "posts": []};    //  Int.Speicher f�r aktuelle Postings *eines* Topics
    var mUser           =   {id:-1};
    var mGroups         =   {};
    var aNews           =   [];     //  enth�lt sortierte neuste themen
    ...
    self.iCurrentMessage =  0;  //  aktuell ge�ffnete PM
    ...
    self.vxSetThreads   = function(m)   {mThreads=m};
    self.vxSetTopic     = function(id,t){mTopics[id]=t};
    self.vxSetPosts     = function(p)   {mPosts=p};
    ...
    //  Einen Post l�schen (letzter Post l�scht auch Topic)
    ...
    self.vxMoveTopic    =   function(iTopicID, bConfirmed) {
        if (bConfirmed) {
            vxCloseOverlay();
            self.call([{"cmd": "moveTopic", "topicID": iTopicID, "threadID": iThreadSelectedFromTree}],function(mAction){
                self["oldHash"] =   null;
            ...
            //  Hinweis einblenden
            vxOpenOverlay(self.szxMoveTopicBox(mThreads, self.mxGetTopic(iTopicID), iTopicID), function(){self.vxSelectTree(0)});
        }
    };
    ...
    self.vxBanUser = function(iPostID,pForm) {
        if(typeof pForm=="undefined") {
            ...
            if (!pForm.banreason1.value)
                ...
                self.call([{"cmd": "banUser", "postID": iPostID, "userID":pForm.userid.value, "banreason1": pForm.banreason1.value, "banreason2": pForm.banreason2.value, "banduration": pForm.banduration.value}],function(mAction){
                    vxCloseOverlay();
                });
        }
    };
    ...
    self.vxEditTopic    =   function(iTopicID, bConfirmed, szHeadline, pIcon) {
        if (bConfirmed) {
            ...
            vxCloseOverlay();
            self.call([{"cmd": "editTopic", "topicID": iTopicID, "headline": szHeadline, "icon": iIcon, "flags":$("js_tmp_pinned").checked?dft_pinned:0}],function(mAction){
                self["oldHash"] =   null;
                location.hash   =   "!showTopic~"+iTopicID+"~"+iCurrentPage;
                self.loadFromHash();
            });
            ...
            vxOpenOverlay(self.szxEditTopicBox(iTopicID), function(){$("js_tmp_subject").value = mTopic.headline; $("js_tmp_icon"+mTopic.icon).checked=true;$("js_tmp_pinned").checked=(mTopic.flags&dft_pinned)?true:false;$("js_tmp_subject").focus()});
        }
    };
    ...
    function getXHR(a){for(a=0;a<4;a++)try{return a?new ActiveXObject([,"Msxml2","Msxml3","Microsoft"][a]+".XMLHTTP"):new XMLHttpRequest}catch(e){}};
    ...
    self.vxCreateTopic =   function(iThreadID, pForm) {
        ...
        //  Falls icon==100 dann Umfrage pr�fen:
        if (iIcon==100) {
        ...
        //  Beitrag abschicken
        self.call([{"cmd": "createTopic", "threadID": iThreadID, "headline": pForm.topic.value, "icon": iIcon, "content": pForm.obj_bb_content0.value, "pollchoosable": iPollOptions, "pollruntime": iPollRuntime, "polloptions": aPollOptions}],function(mAction){
            if (mAction.action=="showTopic")
                location.hash = "!showTopic~"+mAction.topicID+"~"+mAction.page+"~"+mAction.postID;
        });
    }
            ...
            for(var x=0;x<amJSON.length;x++) {
                ...
                switch(evnt) {
                    ...
                    case "newThreads":
                        ...
                        //  Forum sortieren
                        for(var key in data) data[key].childs.sort(function(a,b){return data[a].sortid-data[b].sortid;});
                        ...
                        aNews.sort(function(a,b){return b.lastpostdate-a.lastpostdate});
                        //  5 News aufbauen:
                        var szNews = "";
                    ...
                    case "newMessageCount":
                        ...
                        //  Falls User ein Popup m�chte, dieses nun �ffnen.
                    ...
                    case "newUser":
                        ...
                        if (mUser["flags"]&dfu_openpm && mUser["messages"]>0)
                            self.vxAlertNewMessages();
                        //  Falls User ver�ndert, Seite neu laden:
    ...
    //  Navigation funktioniert ausschliesslich �ber hash
...
if(param[0]!="showTopic")
    mPosts=null; // gecachte posts l�schen
            ...
            switch(param[0]) {
                ...
                case "showTopic":
                    if (!mPosts || mPosts["topicID"] != intval(param[1]) || iCurrentPage!=intval(param[2])) {
                        self.call([{"cmd": "getPosts", "page": intval(param[2]), "topicID": intval(param[1]), "postID": intval(param[3])}], function(mAction){
    ...
    self.vxLogin    =   function() {
        vxOpenOverlay(self.szxLoginBox(), function(){$("js_tmp_login").focus()});
    }
    ...
    self.vxSaveProfile = function(pForm) {
        ...
        self.call([{"cmd": "saveProfile", "userID": pForm.uid.value, "signature": signature, "flags": flags, "nick": nick, "titel": titel, "email": email}], function(mAction){
            if (mAction.action=="OK") {
                vxCloseOverlay();
            }
        });
    ...
    self.vxSendMessage = function(pForm, cb) {
        try {var refid   = intval(pForm.refid.value);} catch(e){var refid=0;}
        ...
        if(!subject)
            ...
            //  Absenden:
            self.call([{"cmd": "writePM", "userID": userid, "subject": subject, "message": message, "refID":refid}],cb);
        }
    };
    ...
    self.vxShowModOptions = function(value) {
        if (value) {
            ...
            $("js_bt_deletemod").onclick = function() {
              try {
                //  �ber Moderatoren iterieren und Eintrag l�schen

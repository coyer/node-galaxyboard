...
module.exports = function GalaxyBoard(config) {
    ...
    self.processCommands = function(req, res, onFinish) {
        ...
        function initBoard(next) {
            ...
            async.series([
                ...
                function getBoard(cb) {
                    var mCachedBoard    =   {0: {"id": 0,   "pid": -1, "headline": "Start",  "childs": [], "perms": {"modflags": 0, "boardflags": 0, "extendflags": 0}, "reports":[], "bIsMod": false, "bManageBans": false}}
                    mysqlPool.query(
                        "select" +
                            ...
                            for (var key in mCachedBoard) {
                                ...
                                if (mModlist[key]) {
                                    for(var mMod in mModlist[key]) {
                                        ...
                                        mCachedBoard[key]["modlist"].push({"id":mMod["userid"], "name": mMod["name"], "flags": mMod["flags"]});
                                    }
                                }
                                ...
                                if (mGrouplist[key]) {
                                    ...
                                    if (arrMenge.length || (mModlist[key] && mModlist[key][mUser["id"]])) {
                                        ...
                                        mBoard[key]["perms"] =   {"modflags": iModFlags, "boardflags": flags.board, "extendflags": flags.extended};
                ...
                function(err, results, fields) {
                    if (err) {
                        ...
                        results.forEach(function(mTopic) {
                            ...
                            mTopic["perms"] =   {"modflags": iModFlags, "topicflags": mTopic["flags"], "boardflags": flags.board, "extendflags": flags.extended};
                                    ...
                                    mysqlPool.query(
                                        ...
                                        function(err, results, fields) {
                                            if (results.length) {
                                                ...
                                                amJSON.push( {"event": "newPosts", "data": {"topicID": iTopicID, "topic": mTopic, "posts": amPosts, "postcount": iPostCount, "page": iPage+1, "poll": null}} )
                                                return next();
                                            }
                                        }
                                    )
                                            ...
                                            function(err,info) {
                                                ...
                                                mysqlPool.query(
                                                    ...
                                                    function(err,info) {
                                                        initBoard(function() {
                                                            amJSON.push( {"event": "newThreads", "data": mBoard} )
                                                            amJSON.push( {"event": "showInfo", "data": "Topic was created!"})
                                                            //  NextAction setzen:  XXX sollte nach den POSTS kommen!
                                                            amJSON.push( {"event": "action", "action": "showTopic", "postID": iPostID, "topicID": iTopicID, "page": 1} );
                                        ...
                                        mysqlPool.query("select count(*) as anzahl from posts where topicid=?",[iTopicID], function(err,results) {
                                            ...
                                            if (!results || results[0].anzahl==0) {
                                                ...
                                                mysqlPool.query(
                                                    "delete from topics where topicid=?",[iTopicID],
                                                    function(err,info) {
                                                        amJSON.push( {"event": "action", "action": "showThread", "threadID": mTopic["boardid"]} );
                                                        next();
                                                    }
                                                );
                                            } else {
                                                ...
                                                self.getPosts({topicID: iTopicID, page:iPage}, function() {
                                                    amJSON.push( {"event": "action", "action": "showTopic", "postID": 0, "topicID": iTopicID, "page": iPage});
                                                    next();
                                                });
                                            }
                                        });
                        ...
                        mysqlPool.query(
                            ...
                            function(err,info) {
                                ...
                                mysqlPool.query(
                                    ...
                                    function(err,info) {
                                        //  Eine gute Idee w�re es wenn ein Beitrag 10x gemeldet wurde diesen auszublenden
        ...
        self.writePM = function(params, next) {
            ...
            //  Messageobject:
            var msg = {"username": mUser["nick"], "titel":mUser["titel"], "userid":mUser["id"], "posts":mUser["posts"], "created":mUser["created"], "content": message, "subject": subject};
                            ...
                            if (result["flag"]&dfmsg_jsondata) {
                                ...
                                mysqlPool.query(
                                    ...
                                    function(err,results) {
                                        if (results.length) {
                                            mTmp = results.shift();
                                            msg = {"username": mTmp["nick"], "titel":mTmp["titel"], "userid":mTmp["id"], "posts":mTmp["posts"], "created":mTmp["created"], "content": result["message"], "subject": result["subject"]}
                                        } else {
                                            msg = {"username": "-unknown-", "titel":"", "userid":0, "posts":0, "created":0, "content": result["message"], "subject": result["subject"]}
                                        }
            ...
            mysqlPool.query("update board_config set headline=?, boardflags=?, description=? where boardid=?",[szName,iBoardFlags,szDescription,iBoardID],
                function(err,info) {
                    ...
                    while(1) {
                        ...
                        if(iTest==iBoardID) {bNested=true;break;}
                        //console.log(["test",iTest,mBoard[iTest].headline]);
                        iTest = mBoard[iTest].pid;
                    }
                    if(bNested) {
                        amJSON.push({"event": "showError", "data": "Nested Error - Board cannot be a child of its own."});
                        return initBoard( function() {self.getThreads(null, next)} );
                    }
                                ...
                                function(err,info) {
                                    function saveMod() {
                                        if (aModlist.length) {
                                            ...
                                            initBoard( function() {self.getThreads(null, next)} );
                                        }
                                    }
                                    saveMod();
                                }
            ...
            mysqlPool.query("select a.*,c.id,c.nick,unix_timestamp(c.created) as created,c.titel,c.posts from groups a left join group_members b on b.groupid=a.groupid left join users c on c.id=b.userid",
                function(err, results){
                    ...
                    for(var i=0;i<results.length;i++) {
                        ...
                        if (obj.id)
                            mGroups[obj.groupid].members.push({"id":obj.id, "nick":obj.nick, "titel":obj.titel, "posts":obj.posts, "created":obj.created });
                    }

...
install_nodejs() {
  ...
  if needs_resolution "$version"; then
    ...
    local version=$(curl --silent --get --data-urlencode "range=${version}" https://semver.io/node/resolve)
  fi

  echo "Downloading and installing node $version..."
  local download_url="http://nodejs.org/dist/v$version/node-v$version-$os-$cpu.tar.gz"
...
install_iojs() {
  ...
  if needs_resolution "$version"; then
    ...
    version=$(curl --silent --get --data-urlencode "range=${version}" https://semver.io/iojs/resolve)
  fi
...
install_npm() {
  ...
  if [ "$version" == "" ]; then
    ...
    if needs_resolution "$version"; then
      ...
      version=$(curl --silent --get --data-urlencode "range=${version}" https://semver.io/npm/resolve)
    fi
